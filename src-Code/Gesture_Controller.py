...
from torchvision import models, transforms  # Import transforms
...
class Controller:
    ...
    #Zoom operations
    def zoom_control(hand_result):
        thumb_tip = hand_result.landmark[4]  # Thumb tip
        index_tip = hand_result.landmark[8]  # Index finger tip
        # Compute Euclidean distance between thumb and index
        pinch_distance = math.sqrt((thumb_tip.x - index_tip.x) ** 2 + (thumb_tip.y - index_tip.y) ** 2)
        # Threshold values to trigger zoom actions
        zoom_in_threshold = 0.1  # Adjust as needed
        zoom_out_threshold = 0.05  # Adjust as needed
    ...
    def changesystembrightness():
        ...
        # Ensure brightness remains within valid range [0, 1]
        currentBrightnessLv = min(1.0, max(0.0, currentBrightnessLv))

        # Set the brightness, using fade_brightness if desired
        new_brightness = int(100 * currentBrightnessLv)
        sbcontrol.fade_brightness(new_brightness, start=int(brightness_values[0]) if isinstance(brightness_values, list) else int(brightness_values))
        ...
        with mp_hands.Hands(max_num_hands = 2,min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            ...
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                if not success:
                    ...
                    continue
                if image is None:
                    print("❌ ERROR: Frame not captured properly. Check your camera.")
                    exit()

                image = cv2.flip(image, 1)  # Flip the image for correct orientation
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert to RGB
                image.flags.writeable = False  # Optimize for MediaPipe processing
                results = hands.process(image)  # Process the frame with MediaPipe
